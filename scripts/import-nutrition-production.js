...
// Production Configuration - Optimized for large datasets
const CHUNK_SIZE = 5000; // 5000 records per chunk (balanced for performance)
const MAX_RETRIES = 3; // Retry failed chunks
const DELAY_BETWEEN_CHUNKS = 1000; // 1 second delay between chunks to prevent overwhelming DB
const PROGRESS_INTERVAL = 10000; // Show progress every 10,000 records
...
async function importNutritionChunk(nutritionRecords, chunkNumber, totalProcessed) {
  console.log(`\n📦 Processing Chunk ${chunkNumber} (${nutritionRecords.length} records)...`);
...
async function importNutritionProduction() {
  console.log('🚀 Starting PRODUCTION Nutrition Import from USDA CSV');
  ...
  console.log('⚠️  This will process ALL 26.8M+ nutrition records...');
    ...
    const stream = fs.createReadStream(csvPath)
      ...
      .on('data', async (row) => {
        ...
        if (totalProcessed % PROGRESS_INTERVAL === 0) {
          ...
          console.log(`📈 Progress: ${totalProcessed.toLocaleString()} records processed (${rate} records/sec)`);
        }
      ...
      .on('end', async () => {
        ...
        console.log('\n🎉 PRODUCTION Nutrition Import Complete!');
        ...
        if (totalErrors === 0) {
          console.log('🎯 100% Success Rate! All nutrition data imported successfully.');
        } else {
...
console.log('🚨 WARNING: This will import ALL 26.8M+ nutrition records!');
console.log('⏳ Estimated time: 2-4 hours depending on your connection');
console.log('💡 You can stop at any time with Ctrl+C and resume later\n');

importNutritionProduction()
  .then((result) => {
    console.log('\n✅ Production nutrition import completed successfully!');
    console.log('💡 Next: Test your LEFT JOIN views with real nutrition data!');
    console.log('🔍 Test command: SELECT * FROM v_food_macros_100g LIMIT 5;');
  })
  .catch((error) => {
    console.error('💥 Production nutrition import failed:', error.message);
